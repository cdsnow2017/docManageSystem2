!function(angular, window, b) {
	...
	app.config([ '$routeProvider', function($routeProvider) {
		...
		// controller: "personalCenterController"
			...
			templateUrl : "./public/template/autadd.html",
			controller : "autController"
		}).when("/myDoc", {
...
//			}
			...
			$scope.data.sectionName = response.sectionName;
	...
	app.controller('simpleDocController', function($scope, $http) {
		...
		/*
		 * $scope.download = function() { $http({ url: "./doc/download", method:
		 * "POST", params: { resourceId: $scope.resNum }, headers : {
		 * 'Content-Type': 'application/x-www-form-urlencoded' }
		 * }).success(function(response) {
		 * 
		 * }).error(function(response) { alert('shabi'); }) }
		 */
	});
